...
export function ForgotPasswordForm() {
  ...
  return <form action={submit}><FieldGroup><Field><FieldLabel htmlFor="recovery-email">Gmail address</FieldLabel><Input id="recovery-email" name="email" type="email" autoComplete="email" placeholder="you@gmail.com" pattern="[A-Za-z0-9._%+-]+@gmail[.]com" title="Use a Gmail address ending in @gmail.com" required /></Field><Button type="submit" disabled={pending}>{pending && <Spinner data-icon="inline-start" />}{pending ? "Sending instructions…" : "Send recovery instructions"}</Button><FieldDescription className="text-center"><Link href="/sign-in" className="font-medium text-primary underline-offset-4 hover:underline">Back to sign in</Link></FieldDescription></FieldGroup></form>;
}
