...
export async function sendTemporaryCredentialsEmail({ email, name, temporaryPassword }: { email: string; name: string; temporaryPassword: string }) {
  const { from, transporter: mailer } = mailTransport();
  const origin = (process.env.NEXT_PUBLIC_APP_URL ?? process.env.BETTER_AUTH_URL ?? "https://easymail.almareem.com").replace(/\/$/, "");
  await mailer.sendMail({
    from,
    to: email,
    subject: "Your temporary Easymail credentials",
    text: `Hello ${name},\n\nAn administrator created an Easymail account for you.\n\nEmail: ${email}\nTemporary password: ${temporaryPassword}\n\nSign in at ${origin}/sign-in. You will be required to replace this password immediately.`,
    html: `<!doctype html><html lang="en"><body style="margin:0;background:#f5f5f4;font-family:Arial,sans-serif;color:#171717"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="padding:32px 14px"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;border:1px solid #e7e5e4;border-radius:20px;background:#fff;overflow:hidden"><tr><td style="padding:24px 32px;background:#111">${emailLogo()}</td></tr><tr><td style="padding:38px 32px;color:#57534e;font-size:16px;line-height:1.7"><h1 style="margin:0 0 20px;color:#171717;font-size:28px">Your account is ready</h1><p>Hello ${escapeHtml(name)},</p><p>An administrator created an Easymail account for you. Use the temporary credentials below once.</p><div style="margin:24px 0;padding:18px;border:1px solid #e7e5e4;border-radius:12px;background:#fafaf9"><strong>Email</strong><br>${escapeHtml(email)}<br><br><strong>Temporary password</strong><br><code style="font-size:16px;color:#171717">${escapeHtml(temporaryPassword)}</code></div><p><a href="${escapeHtml(origin)}/sign-in" style="display:inline-block;padding:13px 22px;border-radius:9px;background:#ff9100;color:#fff;text-decoration:none;font-weight:700">Sign in securely</a></p><p style="font-size:13px">You must choose a new password immediately after signing in. Do not forward this email.</p></td></tr></table></td></tr></table></body></html>`,
  });
}
