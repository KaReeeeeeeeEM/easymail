...
export function WorkspaceSwitcher({ organizations, activeOrganizationId }: { organizations: Workspace[]; activeOrganizationId: string | null }) {
  ...
  async function createWorkspace(formData: FormData) {
    if (atLimit) return toast.error("You can create a maximum of 5 workspaces.");
    setPending(true);
    ...
    try {
      ...
      const selected = await authClient.organization.setActive({ organizationId: result.data.id });
      if (selected.error) return toast.error("Workspace created, but it could not be opened.");
  ...
  return <>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" sideOffset={6} className="min-w-64">
            <DropdownMenuGroup>
              ...
              {organizations.map((organization) => <DropdownMenuItem key={organization.id} disabled={pending} onClick={() => void switchWorkspace(organization.id)}>
                <Building2 />
            ...
            <DropdownMenuGroup>
              <DropdownMenuItem disabled={atLimit || pending} onClick={() => setCreateOpen(true)}><Plus />Create workspace</DropdownMenuItem>
            </DropdownMenuGroup>
            {atLimit && <p className="px-1.5 py-1 text-xs text-muted-foreground">Maximum of 5 workspaces reached.</p>}
          </DropdownMenuContent>
        </DropdownMenu>
      </SidebarMenuItem>
    </SidebarMenu>
    ...
    <Dialog open={createOpen} onOpenChange={(open) => !pending && setCreateOpen(open)}>
      <DialogContent>
        <DialogHeader><DialogTitle>Create workspace</DialogTitle><DialogDescription>Create another workspace for a project or organization. You can create {5 - organizations.length} more.</DialogDescription></DialogHeader>
        <form onSubmit={(event) => { event.preventDefault(); void createWorkspace(new FormData(event.currentTarget)); }}>
          <FieldGroup><Field><FieldLabel htmlFor="sidebar-workspace-name">Workspace name</FieldLabel><Input id="sidebar-workspace-name" name="name" placeholder="Acme Operations" autoFocus required minLength={2} maxLength={80} /></Field><DialogFooter><Button type="submit" disabled={pending}>{pending && <Spinner data-icon="inline-start" />}{pending ? "Creating workspace…" : "Create workspace"}</Button></DialogFooter></FieldGroup>
        </form>
      </DialogContent>
    </Dialog>
  </>;
