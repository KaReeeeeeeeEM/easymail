...
export default async function DashboardPage() {
  ...
  return <div className="flex flex-col gap-8">
    ...
    <Card className="overflow-hidden"><CardHeader><CardTitle>Recent requests</CardTitle><CardDescription>SMTP accepted means the provider accepted the message; it is not an inbox-read receipt.</CardDescription></CardHeader><CardContent className="p-0"><RecentRequestsTable requests={data.recent.map((item) => ({ id: item.id, subject: item.subject, recipients: item.recipients, status: item.status, createdAt: item.createdAt.toISOString() }))} /></CardContent></Card>
  </div>;
}
