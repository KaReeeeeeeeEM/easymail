...
export async function getDashboardData(organizationId: string) {
  ...
  return { metrics: { total: deliveries.length, accepted, failed, acceptanceRate: completed ? Math.round((accepted / completed) * 1000) / 10 : 0, senders: senders.length }, trend, recent: deliveries.slice(0, 8), updatedAt: new Date() };
}
